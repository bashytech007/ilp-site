    ...
    <div className="min-h-screen flex flex-col bg-linen-50 font-body antialiased overflow-x-hidden">
      ...
      <main id="main-content" className="flex-grow pt-32 pb-24 overflow-x-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16 max-w-full">
          ...
          {practice.isFlagship && (
            <div className="bg-charcoal-900 text-white p-5 sm:p-10 rounded-2xl border border-charcoal-800 space-y-4 shadow-xl max-w-full overflow-hidden">
              <Badge variant="terracotta" className="bg-terracotta-500 text-white border-none">
                Flagship Practice Feature
              </Badge>
              <h3 className="font-heading font-bold text-xl sm:text-2xl text-white break-words">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
            {practice.pillars.map((pillar, idx) => (
              <div
                key={pillar.title}
                className="bg-white p-5 sm:p-8 rounded-xl border border-beige-300 shadow-sm space-y-4 flex flex-col justify-between max-w-full overflow-hidden"
              >
                <div className="space-y-3">
                  <div className="text-xs font-bold text-terracotta-500">PILLAR 0{idx + 1}</div>
                  <h3 className="font-heading font-bold text-lg sm:text-xl text-charcoal-900 break-words">{pillar.title}</h3>
                  <p className="text-xs text-slate-600 leading-relaxed break-words">{pillar.description}</p>
                </div>
                ...
                <div className="pt-4 border-t border-slate-100 space-y-2">
                  ...
                  {pillar.services.map((serv) => (
                    <div key={serv} className="flex items-start gap-2 text-xs text-slate-700 font-medium min-w-0">
                      <CheckCircle2 className="h-4 w-4 text-terracotta-500 shrink-0 mt-0.5" />
                      <span className="break-words min-w-0">{serv}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* Regulators List */}
          <div className="bg-white p-5 sm:p-8 rounded-xl border border-beige-300 space-y-3 max-w-full overflow-hidden">
            <h4 className="font-heading font-bold text-base sm:text-lg text-charcoal-900">Key Sector Regulators Mapped</h4>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {practice.regulators.map((reg) => (
                <span key={reg} className="text-xs font-semibold px-2.5 py-1 bg-beige-200 text-charcoal-900 rounded-md border border-beige-300 break-words">
                  {reg}
                </span>
              ))}
            </div>
          ...
          <div className="text-center pt-4">
            <Link href="/contact" className="inline-block w-full sm:w-auto">
              <Button variant="terracotta" size="lg" className="w-full sm:w-auto px-6 text-sm sm:text-base h-auto py-3.5 leading-snug whitespace-normal" rightIcon={<ArrowRight className="h-4 w-4 shrink-0" />}>
